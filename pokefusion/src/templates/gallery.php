...
<html lang="en" data-bs-theme="dark">
    <head>
        ...
        <meta name="description" content="Gallery of Pokéfusion">
        ...
        <meta property="og:url" content="https://cs4640.cs.virginia.edu/xax8gw/sprint4/">
        ...
        <title>Gallery Screen for Pokéfusion</title>
    ...
    <nav class="navbar navbar-expand-md">
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav">
                    <li class="nav-item"><a class="nav-link" href="?command=homeLoggedIn">Home</a></li>
                    <li class="nav-item"><a class="nav-link" href="?command=userProfile">Profile</a></li>
                    <li class="nav-item"><a class="nav-link active" href="?command=gallery">Gallery</a></li>
                    <li class="nav-item"><a class="nav-link" href="?command=play">Play</a></li>
                    <li class="nav-item"><a class="nav-link" href="?command=logout">Log Out</a></li>
                </ul>
            </div>
        </div>
    </nav>
    ...
    <header class="mx-3">
        <h1 class="display-6 pt-3 mt-5"><span class="first-word">POKÉ</span>FUSION Drawing Gallery</h1>
        <hr>
    </header>
    ...
    <nav class="navbar">
        <div class="container-fluid">
            <form class="d-flex flex-column flex-md-row w-100 gap-2" method="GET">
                <input type="hidden" name="command" value="gallery">
    
                <input class="form-control" type="search" name="title" placeholder="Search by title">
                <input class="form-control" type="search" name="username" placeholder="Search by username">
                ...
                <button class="btn btn-error" type="submit">Search</button>
            </form>
        </div>
    </nav>
    ...
    <div id="drawingHolder" class="d-flex justify-content-center align-items-start flex-wrap row mt-3">
        <?php if (empty($drawings)): ?>
            <div class="col-12 text-center"><p class='alert alert-danger'>No drawings found.</p></div>
        <?php else: ?>
            <?php foreach ($drawings as $drawing): ?>
                <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6 d-flex justify-content-center">
                    <div class="card mb-3 drawing-card"
                        ...
                        data-id="<?= $drawing['id'] ?>"
                        data-username="<?= htmlspecialchars($drawing['username']) ?>"
                        style="max-width: 18rem; cursor: pointer;">
                        <div class="card-body text-center">
                            <h5 class="card-title mt-3"><?= htmlspecialchars($drawing['title']) ?></h5>
                            <p class="card-text">By <?= htmlspecialchars($drawing['username']) ?></p>
                            <p>Rating: <?php
    ...
    <div class="container">
        <footer class="py-3 my-4">
            ...
            <p class="text-center text-muted">This website is not affiliated with, endorsed, or sponsored by The Pokémon Company.</p>
        </footer>
    </div>
        ...
        <div class="modal-dialog modal-fullscreen">
            <div class="modal-content">
                ...
                <div class="modal-body text-center">
                    ...
                    <h4 id="drawingModalTitleInfo"></h4>
                    <h5 id="drawingModalUserInfo"></h5>
                    <h5 id="drawingModalRating"></h5>
                </div>
                <div class="text-center">
                    <form method="POST" action="?command=submitRating" class="mb-3">
                        <input type="hidden" name="drawing_id" id="modalDrawingId">
                        <label for="rating" class="form-label">Rate this drawing:</label>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            modalTitleInfo = document.getElementById("drawingModalTitleInfo");
            modalUserInfo = document.getElementById("drawingModalUserInfo");
            ...
            document.querySelectorAll(".drawing-card").forEach(card => {
                card.addEventListener("click", function () {
                    ...
                    modalTitleInfo.innerText = "Fusion: " + title;
                    modalUserInfo.innerText = "By " + username;
                    if (rating != 0) {
                    ...
                    document.getElementById("modalDrawingId").value = this.dataset.id;
